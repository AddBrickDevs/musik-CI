...
class Admin extends CI_Model 
{
    ...
    function setLike($id)
    {
        ...
        return   $this->db->get_where("activity",array("idactivity" => intval($id)));
    ...
    function getActivityUser($id=false,$limit = 50)
    {
        $extra = '';
        if(intval($id)>0)
    ...
    {
        return $this->db->query("SELECT COUNT(*) AS TotalRows,artist,track,picture FROM activity  WHERE action ='1' AND picture != '' AND date > NOW() - INTERVAL 7 DAY  group by artist,track,picture order by TotalRows DESC LIMIT ".INTVAL($this->config->item("items_top")));
    }

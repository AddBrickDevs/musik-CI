<div class="row row-sm padder-lg ">
	...
	<div class="col-xs-6 col-sm-4 col-md-3 col-lg-2">
		<div class="item">
			<div class="pos-rlt">					
				<div class="item-overlay opacity r r-2x bg-black">
					<div class="center text-center m-t-n">
						<a onclick="addPlayList('<?php echo addslashes($value->title->label); ?>','<?php echo addslashes($value->artist->label); ?>','<?php echo $image; ?>',true);"  href="#"><i class="icon-control-play i-2x"></i></a>
					</div>
					<div class="bottom padder m-b-sm">
						<a href="#" onclick="start_radio('<?php echo addslashes($value->title->label); ?>','<?php echo addslashes($value->artist->label); ?>','<?php echo $image; ?>')" class="pull-right">
							<i class="fa fa-rss"></i>
						</a>
						<a href="#" onclick="addPlayList('<?php echo addslashes($value->title->label); ?>','<?php echo addslashes($value->artist->label); ?>','<?php echo $image; ?>',false);">
			...
			<div class="padder-v">
				<a href="#" class="text-ellipsis" onclick="getSongInfo('<?php echo addslashes($value->artist->label); ?>','<?php echo addslashes($value->title->label); ?>');"><?php echo $value->title->label; ?></a>
				<a href="#" class="text-ellipsis text-xs text-muted"  onClick="getArtistInfo('<?php echo $value->artist->label; ?>');" title=<?php echo ___("label_get_artist_info"); ?>><?php echo $value->artist->label; ?></a>
			</div>
		</div>
	</div>
	<?php
